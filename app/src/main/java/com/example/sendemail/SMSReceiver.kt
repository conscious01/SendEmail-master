...
//                    emailContent.append(time + "\n")
            ...
            try {
                Log.v("onReceive", emailContent.toString())
                val address = PreferenceManager.getDefaultSharedPreferences(context)
                    .getString(MainActivity.SP_ADDRESS, "")


                if (address.isEmpty()) {
                    MailUtil.send("_SMS",originatingAddress, emailContent.toString())

                } else {
                    MailUtil.send("_SMS",address, emailContent.toString())
